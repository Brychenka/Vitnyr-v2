...
reveal rhythm fires once per element as it enters view."""
...
def _stroke_matrices(page):
    """The six matrix components for each §02 loop-mark stroke, in DOM order."""
    return page.evaluate(
        """() => [...document.querySelectorAll('.loopmark__stroke')].map(el => {
            const t = getComputedStyle(el).transform;
...
def test_loopmark_strokes_lean_toward_the_pointer_and_return(open_site):
    """§02's mute mark (2026-09-22): three strokes that rest as the exact
    locked logo and lean toward the pointer dot independently — each from its
    own notional centre, so they separate instead of sliding as one figure.
    initLoopmark() in main.js; see the comment block above the figure."""
    page, _ = open_site()
    fig = page.locator(".loopmark")
    fig.scroll_into_view_if_needed()
    page.wait_for_timeout(1200)          # let the reveal rise settle

    at_rest = _stroke_matrices(page)
    assert len(at_rest) == 3, at_rest
    for m in at_rest:
        assert abs(m[4]) < 0.5 and abs(m[5]) < 0.5, f"a stroke must rest at zero ({m})"
    ...
    pulled = _stroke_matrices(page)
    moved = [m for m in pulled if abs(m[4]) > 1.0 or abs(m[5]) > 1.0]
    assert len(moved) == 3, f"all three strokes should lean toward the pointer ({pulled})"
    # they lean independently, not as one logo: the displacements differ
    assert len({(round(m[4], 1), round(m[5], 1)) for m in pulled}) == 3, pulled
    for m in pulled:
        # PULL is 7 and the vector is scaled whole, so a diagonal can't exceed it
        assert (m[4] ** 2 + m[5] ** 2) ** 0.5 <= 7.6, f"lean exceeds the 7px ceiling ({m})"

    page.mouse.move(400, 300)
    page.wait_for_timeout(900)
    for m in _stroke_matrices(page):
        assert abs(m[4]) < 0.5 and abs(m[5]) < 0.5, f"a stroke must settle back ({m})"


def test_loopmark_never_rotates_the_strokes(open_site):
    """reference/vitnyr-brand-identity.md: no stretching, skewing or rotating
    the mark. The lean is translation only — the matrix's a/b/c/d stay identity
    at rest and under the pull alike."""
    page, _ = open_site()
    fig = page.locator(".loopmark")
    fig.scroll_into_view_if_needed()
    ...
    box = fig.bounding_box()
    fig.hover()
    for dx, dy in ((0.15, 0.2), (0.85, 0.25), (0.5, 0.9)):
        page.mouse.move(box["x"] + box["width"] * dx, box["y"] + box["height"] * dy)
        page.wait_for_timeout(250)
        for m in _stroke_matrices(page):
            assert (m[0], m[1], m[2], m[3]) == (1, 0, 0, 1), f"stroke is not upright ({m})"


def test_loopmark_is_inert_under_reduced_motion(open_site):
    """initLoopmark() sits past the reduced-motion return, and the sheet pins
    the strokes flat as well — a reduce reader gets the locked mark, complete
    and visible, and no lean at all."""
    page, _ = open_site(reduced_motion=True)
    fig = page.locator(".loopmark")
    fig.scroll_into_view_if_needed()
    page.wait_for_timeout(400)
    assert fig.evaluate("el => getComputedStyle(el).opacity") == "1"
    box = fig.bounding_box()
    fig.hover()
    page.mouse.move(box["x"] + box["width"] - 3, box["y"] + box["height"] / 2)
    page.wait_for_timeout(600)
    for m in _stroke_matrices(page):
        assert abs(m[4]) < 0.5 and abs(m[5]) < 0.5, f"no lean under reduce ({m})"
